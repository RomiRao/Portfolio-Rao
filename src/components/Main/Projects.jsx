...
export default function Projects() {
  ...
  return (
    <Box display='flex' justifyContent='center' flexWrap='wrap' py={5}>
      ...
      <Card key={project.title} sx={{ maxWidth: 300, m: 3, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <CardMedia
            sx={{ height: 150 }}
            image={project.img}
            title={project.title}
            ...
            <CardActions>
                <Button size="small" href={project.repository} target="_blank">Repository</Button>
                <Button size="small" href={project.deploy} target="_blank">Deploy</Button>
            </CardActions>
      </Card>
      ))}
    </Box>
